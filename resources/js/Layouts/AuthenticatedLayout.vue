...
<template>
  <div class="min-h-screen bg-surface">
    ...
    <nav class="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-surface flex justify-around items-center z-50" style="box-shadow: 0 -2px 12px rgba(146, 76, 0, 0.08);">
      <Link
        v-for="item in mobileNav"
        :key="item.route"
        :href="route(item.route)"
        class="flex flex-col items-center gap-1"
        :class="currentRoute() === item.route ? 'text-primary' : 'text-on-surface-variant'"
      >
